...
`Lựa chọn của bạn:`
*   **Tiếp tục (Nhập `tiep tuc` hoặc nhấn Enter):** Giữ nguyên dữ liệu hiện tại, sau đó lọc các URL có cột B là `Noindex` và cột E có giá trị `Chưa chạy Google Indexing API` để chạy ép index từ trên xuống.
*   **Từ đầu (Nhập `tu dau`):** Hệ thống sẽ mặc định gán giá trị `Chưa chạy Google Indexing API` cho cột E của các URL có cột B là `Noindex`, sau đó lọc và chạy ép index từ trên xuống.

*Lưu ý:* Số lượng giới hạn chạy được thiết lập mặc định tối đa là **200 URLs** cho mỗi lượt và sẽ tự động bắt đầu chạy mà không cần nhập giới hạn hay nhấn Enter thêm.
